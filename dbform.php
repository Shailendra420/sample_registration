...
<body>
	<?php
		$name = $lastname = $userName = $password = $retypePassword = $month = $day = $year = $gender = $mobileNumber = $email = $location = $imageFile = "";
			$nameErr =  "";
			//echo "<pre>";
			//print_r($_REQUEST);print_r($_POST);print_r($_FILES);//exit;
			if ($_SERVER["REQUEST_METHOD"] == "POST"){ //exit;
			  ...
			  /*if ($uploadOk == 0){
				...
				//$row["modify"] = "edit";
				$sql = "INSERT INTO users (firstname, lastname, username, password, modify)
				VALUES ('$name', '$lastname', '$userName', '$password', 'edit')";
			  ...
			  if ($imgFileType !== "jpg" && $imgFileType !== "jpeg" && $imgFileType !== "png" && $imgFileType !== "gif"){
				echo "File should be jpg, jpeg, png or gif extention type." . "<br>";
				$uploadOk = 0;
			  }
			  // Check if set to $uploadOk to 0.
				...
				//header('location:dbform.php');
					...
					if (move_uploaded_file($_FILES["imageFile"]["tmp_name"], $target_file)){
						echo "The file ". basename($_FILES["imageFile"]["name"]) . " is successfully uploaded." . "<br>";
						echo "New record created successfully" . "<br>";
						$last_id = mysqli_insert_id($conn);
						echo $last_id;
						$sql = "UPDATE users SET imagefile= '$target_file' WHERE id=".$last_id;
						...
						//$sql = "INSERT INTO users (imagefile)
						//VALUES ('$target_file')";
						
					} else {
						echo "Sorry, there was an error uploading your file" . "<br>";//exit;
					}
				}
						...
						<form class="width" id="validate-form" method="post" action='' enctype="multipart/form-data">
							...
							<div class="form-group row">
								<div class="col-md-12 col-xs-12">
									<div><input type="text" class="form-control common form-text" id="form-username" name="userName" value="<?php echo $userName;?>"></div>
									<span class="atgmail">@gmail.com</span>
									<div><p class="error-text" id="error-username"></p></div>
								</div>
							</div>
							<div class="row">
							...
							<div class="form-group row">
								<div class="col-md-2 col-md-push-8 col-xs-2 col-xs-push-9">
									<input type="submit" name="submit" class="btn btn-primary form-cotrol" id="submit" value="Next step">
